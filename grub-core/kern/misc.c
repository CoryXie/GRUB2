...
* @note 注释详细内容:
*
* 由于GRUB2本身是boot loader，本身不像在标准操作系统中那样有标准的输入输出库，因此输出格
* 式化打印信息的函数必须自己实现（grub_printf()就是GRUB2自己实现的格式化输出函数）。
*
* 这里我们说明这个grub_printf()的实现，但是要明白这个实现，需要我们理解C语言如何实现可变
* 参数函数。下面简单介绍可变参数函数的参数传递原理。
* 
...
* 
* 那么，调用函数的时候，实参 char z 先进栈，然后是 float y，最后是 int x，因此在内存中变量
* 的存放次序是 x->y->z，因此，从理论上说，我们只要探测到任意一个变量的地址，并且知道其他变
* 量的类型，通过指针移位运算，则总可以顺藤摸瓜找到其他的输入变量。
* 
* 下面是 <stdarg.h> 里面重要的几个宏定义如下（GCC自带有<varargs.h> 和<stdarg.h> 的实现，对
* 于在栈上传递参数的机器无需做改变，因此这里GRUB2应该是使用了GCC的这一实现，虽然GRUB2不使
* 用操作系统提供的库，但是可以使用编译器提供的函数库。这一点可以通过
* 【grub-2.00/grub-core/kern/misc.c】代码中的#include <stdarg.h>得到确认）： 
* 
...
* 
* 第一步：在调用参数表之前，定义一个 va_list 类型的变量，(假设va_list 类型变量被定义为ap)；
* 
* 第二步：然后应该对ap 进行初始化，让它指向可变参数表里面的第一个参数，这是通过 va_start 来
* 实现的，第一个参数是 ap 本身，第二个参数是在变参表前面紧挨着的一个变量,即“...”之前的那个
* 参数；
* 
* 第三步：然后是获取参数，调用va_arg，它的第一个参数是ap，第二个参数是要获取的参数的指定类型，
* 然后返回这个指定类型的值，并且把 ap 的位置指向变参表的下一个变量位置；
* 
* 第四步：获取所有的参数之后，我们有必要将这个 ap 指针关掉，以免发生危险，方法是调用 va_end，
* 他是输入的参数 ap 置为 NULL，应该养成获取完参数表之后关闭指针的习惯。说白了，就是让我们的
* 程序具有健壮性。通常va_start和va_end是成对出现。
* 
* 有了上面的知识，我们来进一步分析这个grub_printf()函数。自然，这里已经按照上面的第一个定义
* 了va_list ap；同样也按照上面的第二步调用了va_start (ap, fmt)；因此，这时候参数ap就是指向
* 了可变参数表里面的第一个参数。下面再调用grub_vprintf (fmt, ap)，就可以使用ap直接去获得可
* 变参数的值。
*
* 这里grub_vprintf()使用到了va_copy(dest, src)这个宏，其中dest，src的类型都是va_list，
* va_copy()用于复制参数列表指针，将dest初始化为src。因此，这里的ap2也就是跟ap一样的指向了可
* 变参数表里面的第一个参数了。这里有两个核心函数，grub_vsnprintf_real()和grub_xputs()，我们
* 可以分别分析。
**/
int
...
*
* 该代码脚长 ，安装功能逻辑分成几个部分，分别说明如下：
*
* 第一段代码用于从格式化字符串中统计出总共有多少个参数需要输出，代码工作流程如下：
*
* 1）	首先让变量fmt指向参数fmt0，即fmt = fmt0，然后使用fmt来依次扫描该格式化字符串中的每
*     个字符。
* 2）	使用while ((c = *fmt++) != 0)扫描每个格式化字符串中的字符，直到字符串结尾字符为止。
* 3）	如果该字符不是“%”格式控制提示符号，那么就认为这个字符是普通的需要原样输出的字符，从
*     而会继续循环到下一个字符。
* 4）	如果遇到了“%”格式控制提示符号，就开始解析该提示符号后面的字符（即真正的格式控制字符）。
* a)	如果是“-”号，即(*fmt && *fmt =='-')，那么继续看下一个字符；
* b)	如果是数字，即grub_isdigit (*fmt)，那么略过这些数字，直到不是数字；
* c)	如果是“$”符号，“-”号，或者数字，或者“.”，那么都继续看下一个字符；
* 
* 也就是说，上面要略过譬如“%-12$-56.23”这样的控制字符，直到“l”，“ll”，“c”,“C”,…等等支持的
* 格式控制符号，每个这样的格式控制符号都需要对应一个输出参数，也就是说，使用var_arg()应该
* 找到该参数的值，然后将参数值按照格式化控制符号要求的格式输出。
*
* 第二段代码代码完成了如下步骤：
*
* 1）	使用前面得到的需要输出的参数个数count_args，生成一个数组types[count_args]以及
*     args[count_args]（这应该是使用了GCC的扩展特性，生成以变量为维度的数组），这两个数组分别保
*     存每个参数的类型以及参数的值。
* 2）	再让变量fmt指向参数fmt0，即fmt = fmt0，然后使用fmt来依次扫描该格式化字符串中的每个字符。
* 3）	使用while ((c = *fmt++) != 0)扫描每个格式化字符串中的字符，直到字符串结尾字符为止。
* 4）	如果该字符不是“%”格式控制提示符号，那么就认为这个字符是普通的需要原样输出的字符，从而
*     会继续循环到下一个字符。
* 5）	如果遇到了“%”格式控制提示符号，就开始解析该提示符号后面的字符（即真正的格式控制字符）。
* a)	如果是“-”号，即(*fmt && *fmt =='-')，那么继续看下一个字符；
* b)	如果是数字，即grub_isdigit (*fmt)，那么略过这些数字，直到不是数字；
* c)	如果是“$”符号，“-”号，或者数字，或者“.”，那么都继续看下一个字符；
* 
* 也就是说，上面要略过譬如“%-12$-56.23”这样的控制字符，直到“l”，“ll”，“c”,“C”,…等等支持的格式
* 控制符号。对每个这样的格式控制符号，设定对应参数的类型types[curn]。
* 6）	对每个参数，按照前面分析出来的参数类型，使用va_arg()获得参数的实际值。
*
* 第三段代码完成如下步骤：
*
* 1）	让变量fmt指向参数fmt0，即fmt = fmt0，然后使用fmt来依次扫描该格式化字符串中的每个字符。
* 2）	使用while ((c = *fmt++) != 0)扫描每个格式化字符串中的字符，直到字符串结尾字符为止。
* 3）	如果该字符不是“%”格式控制提示符号，那么就认为这个字符是普通的需要原样输出的字符，于是就调
*     用write_char (c)将这个字符写入要返回的缓冲区，进而会继续循环到下一个字符。
* 4）	如果遇到了“%”格式控制提示符号，就开始解析该提示符号后面的字符（即真正的格式控制字符），并
*     相应地调用write_str()，write_fill()和write_char()等内部函数来将格式化好的字符串写入要返回的
*    缓冲区。
**/
